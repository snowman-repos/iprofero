...
angular.module('ui.bootstrap.datetimepicker', [])
  ...
  .constant('dateTimePickerConfigValidation', function (configuration) {
    ...
    var validOptions = ['startView', 'minView', 'maxView', 'minuteStep', 'dropdownSelector'];
  ...
  .directive('datetimepicker', ['dateTimePickerConfig', 'dateTimePickerConfigValidation', function (defaultConfig, validateConfigurationFunction) {
    ...
    return {
      ...
      template: "<div class='datetimepicker'>" +
        "<table class='table-condensed startdate'>" +
        "   <thead>" +
        "       <tr>" +
        "           <th class='left'" +
        "               data-ng-click=\"changeView(data.currentView, data.leftDate, $event)\"" +
        "               ><i class='left arrow icon'/></th>" +
        ...
        "                       data-ng-class='{active: dateValue.active, past: dateValue.past, future: dateValue.future, disabled: dateValue.disabled, today: dateValue.today}' " +
        "                       data-ng-click=\"dateValue.disabled || changeView(data.nextView, dateValue.date, $event)\">{{ dateValue.display }}</span> " +
        "           </td>" +
        "       </tr>" +
        '       <tr data-ng-show=\'data.currentView == "day"\' data-ng-repeat=\'week in data.weeks\'>' +
        "           <td data-ng-repeat='dateValue in week.dates' " +
        "               data-ng-click=\"dateValue.disabled || changeView(data.nextView, dateValue.date, $event)\"" +
        "               class='day' " +
        "               data-ng-class='{active: dateValue.active, past: dateValue.past, future: dateValue.future, disabled: dateValue.disabled, today: dateValue.today}' >{{ dateValue.display }}</td>" +
        "       </tr>" +
        "   </tbody>" +
        "</table>" +
        "</div>",
      ...
      link: function (scope, element, attrs) {
        ...
        var dataFactory = {
          ...
          month: function (unixDate) {
            ...
            var result = {
              'previousView': configuration.maxView === 'month' ? 'setTime' : 'year',
              'currentView': 'month',
            ...
            for (var i = 0; i < 12; i++) {
              ...
              var dateValue = {
                ...
                'disabled': monthMoment.isAfter(moment()),
                'today': monthMoment.format("MM/YYYY") == moment().format("MM/YYYY")
              };
          ...
          day: function (unixDate) {
            ...
            var result = {
              'previousView': configuration.maxView === 'day' ? 'setTime' : 'month',
              'currentView': 'day',
            ...
            for (var i = 0; i < 6; i++) {
              ...
              for (var j = 0; j < 7; j++) {
                ...
                var dateValue = {
                  ...
                  'future': monthMoment.isAfter(endOfMonth) || monthMoment.isAfter(today),
                  'disabled': monthMoment.isAfter(endOfMonth) || monthMoment.isAfter(today) || monthMoment.isBefore(startOfMonth),
                  'today': monthMoment.format("DD/MM/YYYY") == moment().format("DD/MM/YYYY")
                };
          ...
          hour: function (unixDate) {
            ...
            var result = {
              'previousView': configuration.maxView === 'hour' ? 'setTime' : 'day',
              'currentView': 'hour',
